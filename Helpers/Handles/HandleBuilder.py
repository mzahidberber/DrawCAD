...
class HandleBuilder:
    ...
    def __createSplineHandles(self) -> list[BaseHandle]:
        for i in range(0, len(self.__element.points)): self.__handles.append(
            PointMoveHandle(self.__element, self.__elementType, i, self.__snap))
        return self.__handles

    def __createRectangleHandles(self) -> list[BaseHandle]:
        for i in range(0, len(self.__element.points)): self.__handles.append(
            PointMoveHandle(self.__element, self.__elementType, i, self.__snap))
        self.__handles.append(MoveHandle(self.__element, self.__elementType, self.__snap))
        return self.__handles

    def __createEllipseHandles(self) -> list[BaseHandle]:
        self.__handles.append(MoveHandle(self.__element, self.__elementType, self.__snap))
        for i in range(0, 4): self.__handles.append(RadiusHandle(self.__element, i, self.__elementType, self.__snap))
        return self.__handles
