...
class Snap:
    ...
    def __intersectionLineToLine(self,e1:Element,e2:Element):
        ...
        if p is not None: self.__pointList.append(SnapPoint(Point(x=p.x(), y=p.y(), pointTypeId=1), SnapTypes.intersection))
    ...
    def __intersectionLineToCircle(self,e1:Element,e2:Element):
        ...
        if p is not None: self.__pointList.extend(
            list(map(lambda x: SnapPoint(Point(x=x.x(), y=x.y(), pointTypeId=1), SnapTypes.intersection), p)))

    def __intersectionLineToEllipse(self,e1:Element,e2:Element):
        p = GeoMath.findIntersectionPointLineAndEllipse(e1.points[0], e1.points[1],e2.points[0],e2.radiuses[0].value,e2.radiuses[1].value)
        if p is not None: self.__pointList.extend(
            list(map(lambda x: SnapPoint(Point(x=x.x(), y=x.y(), pointTypeId=1), SnapTypes.intersection), p)))
    ...
    def __intersectionCircleToCircle(self,e1:Element,e2:Element):
        ...
        if p is not None: self.__pointList.extend(
            list(map(lambda p: SnapPoint(Point(x=p.x(), y=p.y(), pointTypeId=1), SnapTypes.intersection), p)))

    def __intersectionEllipseToCircle(self,e1:Element,e2:Element):
        p = GeoMath.findIntersectionPointEllipseAndCircle(e1.points[0],e1.radiuses[0].value,e1.radiuses[1].value,e2.points[0],e2.radiuses[0].value)
        if p is not None: self.__pointList.extend(
            list(map(lambda x: SnapPoint(Point(x=x.x(), y=x.y(), pointTypeId=1), SnapTypes.intersection), p)))

    def __intersectionEllipseToEllipse(self,e1:Element,e2:Element):
        p = GeoMath.findIntersectionPointEllipses(e1.points[0], e1.radiuses[0].value,e1.radiuses[1].value,e2.points[0],e2.radiuses[0].value,e2.radiuses[1].value)
        if p is not None: self.__pointList.extend(
            list(map(lambda x: SnapPoint(Point(x=x.x(), y=x.y(), pointTypeId=1), SnapTypes.intersection), p)))
            ...
            for i in range(0, len(elements)):
                for a in range(0, len(elements)):
                    ...
                    match elements[i].elementTypeId, elements[a].elementTypeId:
                        case 1, 1: self.__intersectionLineToLine(elements[i],elements[a])
                        case 1, 2: self.__intersectionLineToCircle(elements[i],elements[a])
                        case 2, 1: self.__intersectionLineToCircle(elements[a],elements[i])
                        case 1, 5: self.__intersectionLineToEllipse(elements[i],elements[a])
                        case 5, 1: self.__intersectionLineToEllipse(elements[a],elements[i])
                        case 1, 4:self.__intersectionLineToCircle(elements[i], elements[a])
                        case 4, 1:self.__intersectionLineToCircle(elements[a], elements[i])
                        case 2, 4:self.__intersectionCircleToCircle(elements[i],elements[a])
                        case 4, 2:self.__intersectionCircleToCircle(elements[a],elements[i])
                        case 2, 2: self.__intersectionCircleToCircle(elements[i],elements[a])
                        case 2, 5: self.__intersectionEllipseToCircle(elements[a],elements[i])
                        case 5, 2: self.__intersectionEllipseToCircle(elements[i],elements[a])
                        case 5, 5: self.__intersectionEllipseToEllipse(elements[i],elements[a])
                        case 5, 4: self.__intersectionEllipseToCircle(elements[i],elements[a])
                        case 4, 5: self.__intersectionEllipseToCircle(elements[a],elements[i])
                        case 4, 4: self.__intersectionCircleToCircle(elements[i],elements[a])
    ...
    def __addLinePoints(self, element: Element, scenePos: QPointF) -> None:
        if self.snapEnd:
            ...
            self.__pointList.extend(list(map(lambda x: SnapPoint(x, SnapTypes.end), points)))
        if self.snapMiddle:
            p = GeoMath.findLineCenterPoint(element.points[0], element.points[1])
            self.__pointList.append(SnapPoint(Point(x=p.x(), y=p.y(), pointTypeId=2), SnapTypes.middle))
        if self.snapNearest:
            p = GeoMath.findPointOnLine(scenePos, element.points[0], element.points[1])
            self.__pointList.append(SnapPoint(Point(x=p.x(), y=p.y(), pointTypeId=1), SnapTypes.nearest))

    def __addCirclePoints(self, element: Element, scenePos: QPointF) -> None:
        if self.snapEnd:
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], element.radiuses[0].value, 0), SnapTypes.end))
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], 0, element.radiuses[0].value), SnapTypes.end))
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], -element.radiuses[0].value, 0), SnapTypes.end))
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], 0, -element.radiuses[0].value), SnapTypes.end))
        if self.snapCenter:
            self.__pointList.append(SnapPoint(element.points[0], SnapTypes.center))
        if self.snapNearest:
    ...
    def __addRectanglePoints(self, element: Element, scenePos: QPointF) -> None:
        if self.snapEnd:
            self.__pointList.extend(list(map(lambda x: SnapPoint(x, SnapTypes.end), element.points)))
        if self.snapMiddle:
            for i in range(0, len(element.points)):
                p = GeoMath.findLineCenterPoint(element.points[i], element.points[i + 1 if i != 3 else 0])
                self.__pointList.append(SnapPoint(Point(x=p.x(), y=p.y(), pointTypeId=3), SnapTypes.middle))
        if self.snapCenter:
            ...
            self.__pointList.append(SnapPoint(Point(x=x, y=y, pointTypeId=2), SnapTypes.center))
        if self.snapNearest:
            pList = []
            for i in range(0, len(element.points)):
    ...
    def __addArcPoints(self, element: Element, scenePos: QPointF) -> None:
        if self.snapEnd:
            self.__pointList.extend(list(map(lambda x: SnapPoint(x, SnapTypes.end), element.points[1:])))
        if self.snapCenter:
            self.__pointList.append(SnapPoint(element.points[0], SnapTypes.center))
        if self.snapNearest:
            p = GeoMath.findPointOnCircleNearest(scenePos, element.points[0], element.radiuses[0].value)
    ...
    def __addEllipsePoints(self, element: Element, scenePos: QPointF) -> None:
        if self.snapEnd:
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], element.radiuses[0].value, 0), SnapTypes.end))
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], 0, element.radiuses[1].value), SnapTypes.end))
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], -element.radiuses[0].value, 0), SnapTypes.end))
            self.__pointList.append(
                SnapPoint(GeoMath.addValueToPoint(element.points[0], 0, -element.radiuses[1].value), SnapTypes.end))
