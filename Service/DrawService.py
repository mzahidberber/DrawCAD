...
class DrawService(object):
    ...
    def startCommand(self, command: CommandEnums, userDrawBoxId: str, userLayerId: str):
        ...
        builder.paramsBuild(f"userName={self.__username}").paramsBuild(
            f"userDrawBoxId={userDrawBoxId}"
        )
    ...
    def getElementsWithItsLayer(self, drawBoxId: int):
        builder = (
            ...
            .urlBuild(self.__url)
            .urlBuild("Element")
            .urlBuild("getElementsWithItsLayer")
        )
        connectionString = (
            builder.paramsBuild(f"userName={self.__username}")
            .paramsBuild(f"drawBoxId={drawBoxId}")
            .build()
        )
